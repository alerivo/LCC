...
void touch(const char *name) {
    if (access("/tmp/grading", F_OK) < 0)
        return;
    char pn[1024];
    /*Este arreglo no se puede desbordad pues está especificado la cantidad de carácteres a copiarle */
...
int http_read_line(int fd, char *buf, size_t size)
{
    ...
    for (;;)
    {
        ...
        if (cc <= 0)
            break;
        //Es necesario agregar && i!= 544 a la condición para
        //poder hacer return-to-libc
        if (buf[i] == '\r')
        {   
            buf[i] = '\0';      /* skip */
            continue;
...
{  
    ...
    {   //Leer linea y guardarla en buf
        ...
        *colon = '\0';
        ...
        /*Toupper cambiar el código hexad:ecimal interno por lo tanto nos impide explotar envvar*/
        ...
        if (strcmp(buf, "CONTENT_TYPE") != 0 &&
            ...
            //Podriamos intentar gobernar esta variable de entorno
...
void split_path(char *pn)
{
    ...
    for (;;) {
        ...
        int r = stat(pn, &st);
        if (r < 0) {
            //Si es un directorio y existe el archivo
...
void http_serve(int fd, const char *name)
{
    ...
    struct stat st;
    getcwd(pn, sizeof(pn));
    //Aca se tiene algo como /home/httpd/http.c
...
void url_decode(char *dst, const char *src, int tam)
{
    int i = 0;
    for (;i < tam;)
    {
